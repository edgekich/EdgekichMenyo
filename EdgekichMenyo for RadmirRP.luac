-- Script head
script_name('EdgekichMenyo for RadmirRP by edgekich')
script_author('edgekich')
script_description('EdgekichMenyo for RadmirRP')
...
local bg_tex = nil
local bg_path = getGameDirectory() .. "\\moonloader\\EdgekichMenyo\\bg.png"
local sw, sh = getScreenResolution()

local function lerp(a, b, t) return a + (b - a) * t end
local function approach(current, target, dt, speed)
    local t = math.min(1.0, dt * speed)
    return lerp(current, target, t)
end
local function mulColor(base, mult, alphaMul)
    alphaMul = alphaMul or 1.0
    return imgui.ImVec4(
        math.min(1.0, base.x * mult),
        math.min(1.0, base.y * mult),
        math.min(1.0, base.z * mult),
        math.min(1.0, base.w * alphaMul)
    )
end
...
UI_MAIN = imgui.ImVec4(120/255, 60/255, 190/255, 0.85)
UI_HOVER = imgui.ImVec4(100/255, 50/255, 160/255, 0.90)
UI_ACTIVE = imgui.ImVec4(153/255, 50/255, 204/255, 0.60)
UI_BORDER = imgui.ImVec4(170/255, 110/255, 255/255, 0.10)
...
fishing_window_state = imgui.ImBool(false)
ohota_window_state = imgui.ImBool(false)
halloween_window_state = imgui.ImBool(false)
diving_window_state = imgui.ImBool(false)
pilot_window_state = imgui.ImBool(false)
obhod_radmir_state = imgui.ImBool(false)
fishing_helper = imgui.ImBool(false)
fishing_helper_buffer = imgui.ImBuffer(256)
fish_value = 0
fishing_combo = imgui.ImInt(0)
fishing_array = {u8"Спиннинг", u8"Поплавочная удочка", u8"Улучшенная удочка"}
fishing_bot = imgui.ImBool(false)
fishing_bot_key = imgui.ImBool(false)
fishing_spamAlt_thread_stop = false
fishing_bot_tunecOff = imgui.ImBool(false)
legit_whohota = imgui.ImBool(false)
legit_whDuck = imgui.ImBool(false)
legit_whDuck_traser = imgui.ImBool(false)
legit_AutoTakeDuck = imgui.ImBool(false)
legit_aimDuck = imgui.ImBool(false)
legit_aimDuck_silent = imgui.ImBool(false)
legit_PilotAutoSkipTable = imgui.ImBool(false)
legit_whDiving = imgui.ImBool(false)
legit_whDiving_traser = imgui.ImBool(false)
legit_Obhod_AC = imgui.ImBool(false)
rage_ygonAvto = imgui.ImBool(false)
fun_gm_window_state = imgui.ImBool(false)
fun_gm = imgui.ImBool(false)
fun_gm1 = imgui.ImBool(false)
tpstate = false
tbpstate = false
tpvehid = 0
tbpvehid = 0
...
ferma1TpPerenos = false
ferma2TpPerenos = false
fermaTpVspah = false
fermaYdobrenie = false
fermaSbor = false
fermaCheckpointPosX = 0.0
fermaCheckpointPosY = 0.0
fermaCheckpointPosZ = 0.0
settings_ActivationInsert = imgui.ImBool(false)
settings_AutoY = imgui.ImBool(false)
settings_AutoY_clicker = false
settings_ObhodAC = imgui.ImBool(false)
legit_halloween_sendListovka = imgui.ImBool(false)
stat_launches_total = imgui.ImInt(0)
stat_launches_today = imgui.ImInt(0)
last_launch_date = ""


local tabAnim = {legit = 0.0, rage = 0.0, settings = 0.0}
...
legit_Obhod_AC = imgui.ImBool(false)
headsize = nil
menu_buttonText_size = nil
menu_HeadFuncText_size = nil
menu_buttonCheatText_size = nil
...
local config = inicfg.load({
    AutoLogin = { AutoLoginStatus = false, Password = "pass" },
    Settings = { ActivationInsert = false, AutoY = false, Obhod_AC = false, RageYgonAvto = false },
    Stats = { TotalLaunches = 0, TodayLaunches = 0, LastDate = "" }
})

stat_launches_total.v = config.Stats.TotalLaunches or 0
stat_launches_today.v = config.Stats.TodayLaunches or 0
last_launch_date = config.Stats.LastDate or ""

local today = os.date("%Y-%m-%d")
if today ~= last_launch_date then
    stat_launches_today.v = 0
    last_launch_date = today
end
stat_launches_total.v = stat_launches_total.v + 1
stat_launches_today.v = stat_launches_today.v + 1

config.Stats.TotalLaunches = stat_launches_total.v
config.Stats.TodayLaunches = stat_launches_today.v
config.Stats.LastDate = last_launch_date
inicfg.save(config)
...
rage_ygonAvto.v = config.Settings.RageYgonAvto or false

-- Update
local dlstatus = require('moonloader').download_status
update_state = false
local script_vers = 131
local script_vers_text = "1.31 - Beta"
...
local script_url = "https://github.com/edgekich/EdgekichMenyo/raw/refs/heads/main/EdgekichMenyo%20for%20RadmirRP.luac"
...
function main()
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateIni.info.vers) > script_vers then
                sampAddChatMessage("Вышла обнова 1.31 - Beta! Необходимые зависимости в тг! {9932cc}EdgekichMenyo {FFFFFF}RadmirRP {FFFFFF}by Edgekich!", -1)
                update_state = true
            end
    ...
    if not update_state then
        if settings_ActivationInsert.v then
            sampAddChatMessage("{9932cc}EdgekichMenyo {FFFFFF}for RadmirRP {FFFFFF}by Edgekich. {ffffff}Загружен!", -1)
    ...
    sampRegisterChatCommand("rbar", function() renbar = not renbar end)
    sampRegisterChatCommand("rstep", function(arg)
        local val = tonumber(arg)
        if val then tpstep = val end
    end)
    sampRegisterChatCommand("rwait", function(arg)
        local val = tonumber(arg)
        if val then tpwait = val end
    end)
    sampRegisterChatCommand("obp", function(arg)
        if isCharInAnyCar(PLAYER_PED) then
            sampAddChatMessage("{5F9EA0}[OBP] {FFFFFF}Работает только с ног", -1)
            return
        end
        if not tbpstate then
            local vehId = tonumber(arg)
            if not vehId then
                local can, id = checkNearbyEmptyVehicle()
                if can then vehId = id else
                    sampAddChatMessage("{5F9EA0}[OBP] {FFFFFF}Рядом нет пустого транспорта", -1)
                    return
                end
            end
            tbpvehid = vehId
            tbpstate = true
            sampAddChatMessage("{5F9EA0}[OBP] {FFFFFF}Обход на телепорт: включён | ID: " .. tbpvehid, -1)
            
            lua_thread.create(function()
                while tbpstate do
                    if not tpstate then
                        local x, y, z = getCharCoordinates(PLAYER_PED)
                        sendUnoccupiedSync(x, y, z - 10.0, tbpvehid)
                        sendPassengerSync(x, y, z - 10.0, tbpvehid, 0)
                    end
                    wait(tpwait)
                end
            end)
            ...
            tbpstate = false
            sampAddChatMessage("{5F9EA0}[OBP] {FFFFFF}Обход на телепорт: выключен", -1)
        end
    end)
    sampRegisterChatCommand("rtp", function(arg)
        if isCharInAnyCar(PLAYER_PED) then
            sampAddChatMessage("{5F9EA0}[RTP] {FFFFFF}Работает только с ног", -1)
            return
        end
        local vehId = tonumber(arg)
        ...
        if tbpstate then
            vehId = tbpvehid
            sampAddChatMessage("{5F9EA0}[RTP] {FFFFFF}Обход включен на транспорт с ID: " .. vehId, -1)
        elseif not vehId then
            local can_tp, id = checkNearbyEmptyVehicle()
            if can_tp then vehId = id else
                sampAddChatMessage("{5F9EA0}[RTP] {FFFFFF}Рядом нет пустого транспорта", -1)
                return
            end
        end
        
        lua_thread.create(function()
            if tpstate then tpstate = false wait(50) end
            local modulehandle = getModuleHandle("clientside.dll")
            if modulehandle == 0 then return end
            local ptr = mem.getint32(modulehandle + 0x431110)
            local tx, ty = mem.getfloat(ptr + 0x10), mem.getfloat(ptr + 0x14)
            local tz = getGroundZFor3dCoord(tx, ty, 1000.0)
            targetPos = {x = tx, y = ty, z = (tz == 0 and 15.0 or tz)}
            tpvehid = vehId
            tpstate = true
            sampAddChatMessage("{5F9EA0}[RTP] {FFFFFF}Взяли транспорт с ID: " .. tpvehid, -1)
            local pX, pY, pZ = getCharCoordinates(PLAYER_PED)
            while tpstate do
                local dist = getDistanceBetweenCoords2d(pX, pY, targetPos.x, targetPos.y)
                if dist < 15.0 then break end
                local angle = math.atan2(targetPos.y - pY, targetPos.x - pX)
                pX = pX + math.cos(angle) * tpstep
                pY = pY + math.sin(angle) * tpstep
                syncAll(pX, pY, pZ, tpvehid)
                wait(tpwait)
            end
            if tpstate then
                wait(50)
                setCharCoordinates(PLAYER_PED, targetPos.x, targetPos.y, targetPos.z + 1)
                local res, vehHandle = sampGetCarHandleBySampVehicleId(tpvehid)
                if res and doesVehicleExist(vehHandle) then
                    setCarCoordinates(vehHandle, targetPos.x + 5, targetPos.y, targetPos.z + 5)
                end
                wait(100)
                tpstate = false
                sampAddChatMessage("{5F9EA0}[RTP] {FFFFFF}Прибыли на метку", -1)
            end
        end)
    end)
	...
	imgui.Process = false
        -- Запускаем основной цикл в отдельной функции, чтобы уменьшить upvalues
...
local targetAlpha = main_window_state.v and 1.0 or 0.0
menuAlpha = approach(menuAlpha, targetAlpha, dt, 6.0) -- 6.0 = скорость появления (меньше = медленнее)
	local UI_TAB     = ImVec4(120/255, 60/255, 190/255, 0.80)
local UI_TAB_HOV = ImVec4(100/255, 50/255, 160/255, 0.90)
local UI_TAB_ACT = ImVec4(75/255,  38/255, 130/255, 1.00)
	-- Главное окно
    if main_window_state.v then
		-- Настройки окна
		imgui.SetNextWindowSize(imgui.ImVec2(650, 450), imgui.Cond.FirstUseEver)
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.PushStyleColor(imgui.Col.WindowBg, imgui.ImVec4(0, 0, 0, 0)) -- фон окна прозрачный
		--colors[clr.WindowBg] = ImVec4(RGBA(0, 0, 0, 0.95))
		local mainColorChild = ImVec4(RGBA(0, 0, 0, 0.15))

		-- Вывод окна
		imgui.PushStyleVar(imgui.StyleVar.Alpha, menuAlpha)
		imgui.Begin('EdgekichMenyo for RadmirRP', main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar)
		apply_custom_style()
		-- ФОН В ОКНЕ (под всеми кнопками)
if bg_tex ~= nil then
    local p = imgui.GetWindowPos()
    local s = imgui.GetWindowSize()
    local dl = imgui.GetWindowDrawList()
	local a = math.floor(255 * menuAlpha)
local col = bit.bor(bit.lshift(a, 24), 0xFFFFFF)
    dl:AddImage(bg_tex, imgui.ImVec2(p.x, p.y), imgui.ImVec2(p.x + s.x, p.y + s.y), imgui.ImVec2(0, 0), imgui.ImVec2(1, 1), col)
end

		--Голова окна
		colors[clr.Border] = ImVec4(RGBA(160, 90, 255, 0.0))
		createChild("", 645, 445, 2, 2, ImVec4(RGBA(160, 90, 255, 0.00)))
			createChild("", 635, 435, 5, 5, ImVec4(RGBA(160, 90, 255, 0.0)))
				createChild("", 625, 425, 5, 5, ImVec4(RGBA(160, 90, 255, 0.0)))
					colors[clr.Border] = ImVec4(RGBA(160, 90, 255, 0.1))
					createChild("", 620, 420, 2, 3, ImVec4(RGBA(0, 0, 0, 0.15)))
						apply_custom_style()
							imgui.SetCursorPosX(590)

						colors[clr.Button]        = ImVec4(RGBA(160, 90, 255, 0.85))
						colors[clr.ButtonHovered] = ImVec4(RGBA(160, 90, 255, 0.25))
						colors[clr.ButtonActive]  = ImVec4(RGBA(160, 90, 255, 0.50))
						...
						colors[clr.Button]        = UI_MAIN
						colors[clr.ButtonHovered] = UI_HOVER
						colors[clr.ButtonActive]  = UI_ACTIVE
						imgui.PushFont(menu_buttonText_size)

local dt = imgui.GetIO().DeltaTime

local function TabButton(id, label, isActive)
    local target = isActive and 1.0 or 0.0

    local a = tabAnim[id]
    local col = mulColor(UI_TAB, 1.0 - 0.25 * a)
    col.w = lerp(UI_TAB.w, UI_TAB_ACT.w, a)

    imgui.PushStyleColor(imgui.Col.Button, col)
    imgui.PushStyleColor(imgui.Col.ButtonHovered, UI_TAB_HOV)
    imgui.PushStyleColor(imgui.Col.ButtonActive, UI_TAB_ACT)

    local pressed = imgui.Button(label, imgui.ImVec2(120, 50))
    local hovered = imgui.IsItemHovered()

    imgui.PopStyleColor(3)

    if (not isActive) and hovered then
        target = 0.6
    end

    tabAnim[id] = approach(tabAnim[id], target, dt, 3.0)
    return pressed
end

if TabButton("legit", u8"Легит", main_menu_legit) then
    main_menu_exitAll()
    main_menu_legit = true
end

if TabButton("rage", u8"Рэйдж", main_menu_rage) then
    main_menu_exitAll()
    main_menu_rage = true
end

if TabButton("settings", u8"Настройки", main_menu_settings) then
    main_menu_exitAll()
...
imgui.PopFont()
						...
						if main_menu_legit then -- Легит
							...
							imgui.SetCursorPosX(25)
							if imgui.Button(u8"Авто-Логин", imgui.ImVec2(202, 50)) then
							...
							imgui.PopFont()
							imgui.Text(u8"Машинист: /mashbot")
								imgui.Text(u8"КарАнлокер(визуал) - X+U")
								imgui.Text(u8"КарВХ - /whcar")
								imgui.Text(u8"СвалкаВХ - /svalka")
								imgui.Text(u8"")
								imgui.Text(u8"")
								imgui.Text(u8"")
								imgui.Text(u8"")
						...
						if main_menu_rage then -- Рэйдж
							...
							imgui.SetCursorPosY(15)
							--Угон авто
if imgui.Checkbox(u8'Угон авто', rage_ygonAvto) then
    config.Settings.RageYgonAvto = rage_ygonAvto.v
    inicfg.save(config)
end
							...
							--Гм
								...
								imgui.Text(u8"Ферма более менее легит: /farmbot")
							-- Починка авто
								imgui.SameLine()
						...
						if main_menu_settings then -- Настройки
							...
							imgui.SetCursorPosX(25)	
								...
								imgui.Separator()
imgui.Text(u8"Статистика запусков EdgekichMenyo:")
imgui.Text(u8"Все время: " .. tostring(stat_launches_total.v))
imgui.Text(u8"Сутки:     " .. tostring(stat_launches_today.v))

imgui.Separator()
imgui.Text(u8"Информация")
if imgui.Button(u8"Канал ТГ", imgui.ImVec2(280, 30)) then
    sampAddChatMessage("{00FF00}Канал ТГ: {FFFFFF}https://t.me/edgekich_radmir", -1)
end

if imgui.Button(u8"Саппорт", imgui.ImVec2(280, 30)) then
    sampAddChatMessage("{00FF00}Саппорт: {FFFFFF}https://t.me/edgekichzxc", -1)
end

if imgui.Button(u8"Репозиторий", imgui.ImVec2(280, 30)) then
    sampAddChatMessage("{00FF00}Репозиторий: {FFFFFF}https://t.me/edgekich_tg", -1)
end
		...
		imgui.EndChild()
		-- ===== СТАТУС-БАР =====
imgui.SetCursorPosY(imgui.GetWindowSize().y - 28)
imgui.Separator()
...
-- версия
imgui.SameLine()
imgui.Text(u8"v" .. script_vers_text)
...
imgui.Text(u8"Radmir RP")
	...
	if fun_gm_window_state.v then
		...
		if imgui.Checkbox(u8'ГМ ( визуал )', fun_gm) then
			if fun_gm.v then
	...
	if obhod_radmir_state.v then
		...
		-- Вывод окна
		imgui.Begin(u8'EdgekichMenyo - Obhod нихуя уже не NEW', obhod_radmir_state, imgui.WindowFlags.NoResize)
...
function apply_custom_style()
    ...
    style.ChildWindowRounding = 18.0
    ...
    colors[clr.FrameBg]        = ImVec4(120/255, 60/255, 190/255, 0.35)
    colors[clr.FrameBgHovered] = ImVec4(120/255, 60/255, 190/255, 0.55)
    colors[clr.FrameBgActive]  = ImVec4(120/255, 60/255, 190/255, 0.75)
    colors[clr.TitleBg]       = imgui.ImVec4(RGBA(90, 40, 140, 0.85))
    colors[clr.TitleBgActive] = imgui.ImVec4(RGBA(120, 70, 200, 0.95))
    colors[clr.TitleBgCollapsed]       = ImVec4(RGBA(139, 0, 255, 0.51))
    colors[clr.CheckMark]     = UI_ACTIVE
    ...
    colors[clr.Separator]        = UI_BORDER
    colors[clr.SeparatorHovered] = UI_HOVER
    colors[clr.SeparatorActive]  = UI_ACTIVE
    ...
    colors[clr.WindowBg] = imgui.ImVec4(0.06, 0.06, 0.06, 0.75)
    colors[clr.ChildWindowBg]          = ImVec4(1.00, 1.00, 1.00, 0.00)
    colors[clr.PopupBg]                = ImVec4(0.08, 0.08, 0.08, 0.94)
    colors[clr.ComboBg]                = colors[clr.PopupBg]
    --colors[clr.Border]                 = ImVec4(0.43, 0.43, 0.50, 0.50)
	colors[clr.Border] = UI_BORDER
...
function imgui.BeforeDrawFrame()
    -- грузим фон 1 раз
    if bg_tex == nil then
        bg_tex = imgui.CreateTextureFromFile(bg_path)
    end
...
function createChild(windowName, x, y, posX, posY, color)
	...
	--colors[clr.ChildWindowBg] = ImVec4(RGBA(170, 110, 255, 0.60))
...
function renderInfoBar()
    if not renbar then return end

    local rX, rY = getScreenResolution()
    local posX, posY = 20, rY / 2 - 50

    local cMain = "{D1D1D1}"
    local cVal  = "{5F9EA0}"

    local info = string.format("%sStep: %s%.1f\n", cMain, cVal, tpstep)
    info = info .. string.format("%sWait: %s%dms\n", cMain, cVal, tpwait)

    local rtpStatus = tpstate and "Teleporting" or (checkNearbyEmptyVehicle() and "Ready" or "No Car")
    info = info .. string.format("%sRTP: %s%s\n", cMain, cVal, rtpStatus)

    local tbpStatus = tbpstate and ("On [" .. (tbpvehid or "?") .. "]") or "Off"
    info = info .. string.format("%sOnFoot Bypass: %s%s", cMain, cVal, tbpStatus)

    -- Только основной текст (без тени, чтобы не было tocolor)
    renderFontDrawText(font, info, posX, posY, -1)
end

function checkNearbyEmptyVehicle()
    local px, py, pz = getCharCoordinates(PLAYER_PED)
    local minDist, foundVeh = 15.0, -1
    for _, v in ipairs(getAllVehicles()) do
        if getDriverOfCar(v) == -1 then
            ...
            if dist < minDist then
                ...
                if res then foundVeh = id end
            end
        end
    end
    return foundVeh ~= -1, foundVeh
end

function onReceiveRpc(id, bs)
    if (tpstate or tbpstate) and id == 12 then return false end
end

function sampev.onSendPlayerSync(data)
    if tpstate then return false end
    if tbpstate then
        data.surfingVehicleId = tbpvehid
        data.surfingOffsets = {999, 999, 999}
    end
end

function sendUnoccupiedSync(x, y, z, vId)
    local data = samp_create_sync_data("unoccupied", false)
    local res, veh = sampGetCarHandleBySampVehicleId(vId)
    if res and doesVehicleExist(veh) then
...
function sendPassengerSync(x, y, z, veh, seat)
    local data = samp_create_sync_data('passenger', true)
    data.position = {x, y, z}
    data.vehicleId = veh
    data.seatId = seat
    data.send()
...
function getCarRealMatrix(handle)
    ...
    if entity ~= 0 then
        local carMatrix = mem.getuint32(entity + 20, true)
        if carMatrix ~= 0 then
            return mem.getfloat(carMatrix + 0, true),
                   mem.getfloat(carMatrix + 4, true),
                   mem.getfloat(carMatrix + 8, true),
                   mem.getfloat(carMatrix + 16, true),
                   mem.getfloat(carMatrix + 20, true),
                   mem.getfloat(carMatrix + 24, true)
        end
    end
    return 0, 0, 0, 0, 0, 0
end

function syncAll(x, y, z, vId)
    if not tpstate then return end
    local rx, ry, rz, dx, dy, dz = 0, 0, 0, 0, 0, 0
    local res, veh = sampGetCarHandleBySampVehicleId(vId)
    if res and doesVehicleExist(veh) then
        rx, ry, rz, dx, dy, dz = getCarRealMatrix(veh)
    end
    local unoc = samp_create_sync_data("unoccupied", false)
    unoc.vehicleId = vId
    unoc.roll = {rx, ry, rz}
    unoc.direction = {dx, dy, dz}
    unoc.position = {x, y, z + 5}
    unoc.send()
    local onfoot = samp_create_sync_data("player")
    onfoot.position = {x, y, z}
    onfoot.surfingVehicleId = vId
    onfoot.surfingOffsets = {0, 0, 5}
    onfoot.specialAction = 4
    onfoot.send()
    local pass = samp_create_sync_data("passenger")
    pass.vehicleId = vId
    pass.position = {x, y, z + 5}
    pass.send()
end
